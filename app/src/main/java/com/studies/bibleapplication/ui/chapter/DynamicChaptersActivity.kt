...
import br.com.valecard.estabelecimento.ui.generic.GenericActivity
...
class DynamicChaptersActivity: GenericActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_chapter_dynamic)
    ...
    private fun receiveParams(){
        intent.getSerializableExtra(KEY_BOOK_SELECTED)?.let {
            ...
            showAlertDialog(message = getString(R.string.st_error_not_receive_chapters)) {
                finish()
            }
        }
    }
    ...
    private fun initSpinner() {
        dynamic_chapter_spinner.adapter = ArrayAdapter(this,
            ...
            titleChapters)
        dynamic_chapter_spinner.onItemSelectedListener(onItemSelected = { adapterView, _, _, _ ->
            adapterView?.let {
    ...
    private fun initFragments(){
        ...
        dynamic_chapter_verses_container.onPageChanged(onPageSelected = {position ->
            selectPage(position)
        })
    }
    ...
    private fun refreshToolbarColor(isItemSelected: Boolean) {
        val colorId = if (isItemSelected){
            myGetColor(R.color.colorLight)
        } else {
            myGetColor(R.color.colorGray)
        }
